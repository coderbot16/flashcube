...
impl LayerV3 {
	fn from_parts(count: i64, parts: (&str, &str, i64)) -> Self {
		LayerV3 { count, namespace: parts.0.to_string(), id: parts.1.to_string(), meta: parts.2 }
	}
}

fn features_to_map(
	input: Vec<(String, Vec<(String, String)>)>,
...
#[cfg(test)]
mod test {
	use super::{FlatV1, FlatV3, LayerV1, LayerV3};
	use std::collections::HashMap;
	...
	fn test_glasscore_v3() {
		...
		features.insert("mineshaft".to_string(), {
			let mut map = HashMap::new();
			map.insert("chance".to_string(), "0.04".to_string());
			map
		});
