	...
	fn from(settings: Customized) -> Self {
		...
		Parts {
			tri: TriNoiseSettings {
				...
				main_scale: Vector3::new(
					h_scale / settings.main_noise_scale_x,
					y_scale / settings.main_noise_scale_y,
					h_scale / settings.main_noise_scale_z,
				),
				y_size: 17,
			},
			height_stretch: settings.height_stretch,
			height: HeightSettings81 {
				coord_scale: Vector3::new(
					settings.depth_noise_scale_x,
					0.0,
					settings.depth_noise_scale_z,
				),
				out_scale: 8000.0,
			...
			ocean: Ocean { top: settings.sea_level, lava: settings.use_lava_oceans },
			...
			decorators: Decorators {
				dungeon_chance: if settings.use_dungeons {
					Some(settings.dungeon_chance)
				} else {
					None
				},
				water_lake_chance: if settings.use_water_lakes {
					Some(settings.water_lake_chance)
				} else {
					None
				},
				lava_lake_chance: if settings.use_lava_lakes {
					Some(settings.lava_lake_chance)
				} else {
					None
				},
				dirt: VeinSettings {
