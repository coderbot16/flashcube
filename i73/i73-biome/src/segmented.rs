...
impl<T> Segmented<T> {
	pub fn new(default: T) -> Self {
		Segmented { segments: Vec::new(), out: default }
	}
	...
	fn segment(&self, at: f64, always_inclusive: bool) -> Option<(&Segment<T>, usize)> {
		...
		for (index, segment) in self.segments.iter().enumerate() {
			if (at < segment.upper)
				|| ((Some(index) == last_idx || always_inclusive) && at == segment.upper)
			{
	...
	pub fn add_boundary(&mut self, upper: f64, value: T) {
		if let Some((seg_upper, index)) =
			self.segment(upper, true).map(|(seg, index)| (seg.upper, index))
		{
			if seg_upper == upper {
...
where
	T: Clone,
{
	pub fn for_all_aligned<A, F>(&mut self, lower: f64, upper: f64, above: &A, on: &F)
	where
		A: Fn() -> T,
		F: Fn(&mut T),
	{
	...
	pub fn align<F>(&mut self, lower: f64, upper: f64, above: &F)
	where
		F: Fn() -> T,
	{
		let split_index =
			self.segment(lower, true).map(|(_, index)| index).unwrap_or(self.segments.len() - 1);
		self.split(split_index, lower, above);

		let end_index =
			self.segment(upper, true).map(|(_, index)| index).unwrap_or(self.segments.len() - 1);
		self.split(end_index, upper, above);
	}

	fn split<F>(&mut self, index: usize, new_boundary: f64, above: &F)
	where
		F: Fn() -> T,
	{
		...
		let (index, value) =
			if before { (index, self.segments[index].value.clone()) } else { (index + 1, above()) };

		self.segments.insert(index, Segment { upper: new_boundary, value })
	}
}

impl<T> Segmented<Segmented<T>>
where
	T: Clone,
{
	// TODO: insert a rect
