...
impl ClimateSource {
	pub fn new(seed: u64, settings: ClimateSettings) -> Self {
		...
		ClimateSource {
			temperature: SimplexOctaves::new(
				&mut Random::new(seed.wrapping_mul(TEMP_COEFF) as u64),
				4,
				settings.temperature_fq,
				0.5,
				(0.4 / scale, 0.4 / scale),
			),
			rainfall: SimplexOctaves::new(
				&mut Random::new(seed.wrapping_mul(RAIN_COEFF) as u64),
				4,
				settings.rainfall_fq,
				0.5,
				(0.8 / scale, 0.8 / scale),
			),
			mixin: SimplexOctaves::new(
				&mut Random::new(seed.wrapping_mul(MIXIN_COEFF) as u64),
				2,
				settings.mixin_fq,
				0.5,
				(4.0 / scale, 4.0 / scale),
...
impl Sample for ClimateSource {
	...
	fn sample(&self, point: Point2<f64>) -> Self::Output {
		...
		let temp = (self.temperature.sample(point) * self.settings.temperature_coeff
			+ self.settings.temperature_mean)
			* self.temp_keep
			+ mixin * self.settings.temperature_mixin;
		let rain = (self.rainfall.sample(point) * self.settings.rainfall_coeff
			+ self.settings.rainfall_mean)
			* self.rain_keep
			+ mixin * self.settings.rainfall_mixin;
...
impl Climate {
	...
	pub fn alpha() -> Self {
		Climate { temperature: 1.0, rainfall: 1.0 }
	}
