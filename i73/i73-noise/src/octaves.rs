...
impl SimplexOctaves {
	pub fn new(
		rng: &mut Random, octaves: usize, fq: f64, persistence: f64, scale: (f64, f64),
	) -> Self {
		...
		for _ in 0..octaves.capacity() {
			octaves.push(Simplex::from_rng(
				rng,
				Vector2::new(scale.0 * frequency, scale.1 * frequency),
				0.55 / amplitude,
			));
...
impl PerlinOctavesVertical {
	pub fn new(
		rng: &mut Random, octaves: usize, scale: Vector3<f64>, y_start: f64, y_count: usize,
	) -> Self {
