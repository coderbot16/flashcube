	...
	fn default() -> Self {
		SimpleLootTable {
			pools: vec![
				...
				Pool::rare(
					Pool::Common { item: Item::Redstone, base_size: 1, add_size: Some(3) },
					2,
				),
...
fn records() -> SimpleLootTable {
	SimpleLootTable { pools: vec![Pool::single(Item::GoldRecord), Pool::single(Item::GreenRecord)] }
}
...
impl Pool {
	...
	fn get_item(&self, rng: &mut Random) -> Option<Stack> {
		match *self {
			...
			Pool::Decide { ref item, ref other, chance } => {
